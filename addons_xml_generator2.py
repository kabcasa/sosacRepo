    ...
    def _generate_addons_file(self):
        ...
        for addon in addons:
            try:
                ...
                #xml_lines = open(_path, "r").read().splitlines() #pour py2
                xml_lines = open(_path, "r",  encoding ="UTF-8").read().splitlines()
                # new addon
            except Exception as e:
                # missing or poorly formatted addon.xml
                print("Excluding 1 %s for %s" % (_path, e))
            try :
                addon_xml = ""
                ...
                try :
                    ...
                    # missing or poorly formatted addon.xml
                    print("erreur  xml.strip %s" % ( e))
            except Exception as e:
                # missing or poorly formatted addon.xml
                print("Excluding 2 %s for %s" % (_path, e))
